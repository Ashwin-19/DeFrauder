...
userIdMapping={}
userIdMappingfile=open("/home/ashwin/Desktop/CollusionEcommerce/datasets/AMAZONM/userIdMapping",'r')
...
productIdMapping={}
productIdMappingfile=open("/home/ashwin/Desktop/CollusionEcommerce/datasets/AMAZONM/productIdMapping",'r')
...
d={}
fake=set()
filee=open("/home/ashwin/Desktop/CollusionEcommerce/datasets/AMAZONM/metadata",'r')
for f in filee:
...
text=[]
filee=open("/home/ashwin/Desktop/CollusionEcommerce/datasets/AMAZONM/reviewContent",'r')
for f in filee:
    fsplit=f.split("\t")
...
for f in filee:
    ...
    r=Review(userid,useridmapped,prodid,prodidmapped,rating,label,datetodays,text[c])
        ...
        for u1i in range(len(userlist)):
            ...
            for u2i in range(u1i+1,len(userlist)):
                ...
                if abs(reviewtime[u1][prod]-reviewtime[u2][prod])<=tau and abs(reviewrating[u1][prod]-reviewrating[u2][prod])<2:
                    ans[u1][u2]=ans[u1][u2]+0.5*suspicion[prod]*((1-(abs(reviewtime[u1][prod]-reviewtime[u2][prod]))/tau)+(1-(abs(reviewrating[u1][prod]-reviewrating[u2][prod]))/2.0))
...
def calc_score(g,Lsub):
    ...
    score=[reviewtightness(g,Lsub),neighbortightness(g,Lsub),producttightness(g),ans[0],ans[1],productreviewerratio(g),groupsize(g),multiplereview(g)]
    score2=score
    score.append(GCS(g))
    score.append(GMCS(g))
    ...
    with open('../../../../baselines/GSBC/YELPNYC/finalgrps.json', 'r') as fp:
        ...
        for grp in grps:
            ...
            if len(grps[grp]['users'])>1:
                ...
                ccgroups[grps[grp]['id']]=sum([grps[grp]['scorepred'][i]*classifier.coef_[0][i] for i in range(8)])/8.0
...
def dcg(r_baseline,r_gt):
    ...
    for g in range(len(r_baseline)):
        ansb=ansb+(r_baseline[g][1]*1.0)/math.log(r_baseline[g][0]+1,2)
...
gt_id=[]
cc_id=[]
sorted_ccgroups=sorted(ccgroups.items(), key=operator.itemgetter(1),reverse=True)
