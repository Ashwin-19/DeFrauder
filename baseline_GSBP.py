...
c=0
fake=set()
filee=open("/home/ashwin/Desktop/CollusionEcommerce/datasets/PLAYSTORE/metadata",'r')
for f in filee:
...
text=[]
filee=open("/home/ashwin/Desktop/CollusionEcommerce/datasets/PLAYSTORE/reviewContent",'r')
for f in filee:
    fsplit=f.split("\t")
...
def averagetimewindow_ratingvariance(group,L):
    ...
    return (avgtw*L)/(1.0*len(group.prods)),(avger*L)/(1.0*len(group.prods)),rating_variance*L
...
def calc_score(g,Lsub):
    ...
    score=[reviewtightness(g,Lsub),neighbortightness(g,Lsub),producttightness(g),ans[0],ans[1],productreviewerratio(g),ans[2],groupsize(g)]
    score2=score
    score.append(GCS(g))
    score.append(GMCS(g))
...
def FindGroups(G,k):
    ...
    if len(G.nodes())>MAXSIZE:
            ...
            if len(list(g.nodes()))>1:
                    ...
                    if len(list(g.nodes()))>MAXSIZE:
                        ...
                        if len(group.users)>1:
                                ...
                                index=len(grps)+1
                                grps[index]={'id':index,'users':list(group.users),'prods':list(group.prods),'scorepred':scorepred, 'scoregt':store, 'scoregtreviewprec':(c*1.0)/denom, 'fakegt':spamicity,'fakepred':0}
		...
		c=0
	    	...
	    	index=len(grps)+1
	        grps[index]={'id':index,'users':list(groupmain.users),'prods':list(groupmain.prods),'scorepred':scorepredmain, 'scoregt':store, 'scoregtreviewprec':(c*1.0)/denom, 'fakegt':spamicitymain,'fakepred':0}
